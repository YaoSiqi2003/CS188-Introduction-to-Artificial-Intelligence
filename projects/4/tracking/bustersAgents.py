...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        pacmanPosition = gameState.getPacmanPosition()
        livingGhostPositionDistributions = [belief for index, belief in enumerate(self.ghostBeliefs)
                                            if gameState.getLivingGhosts()[index + 1]]

        # Step 1: Identify the most likely position of each living ghost
        ghostPositions = []
        for distribution in livingGhostPositionDistributions:
            mostLikelyGhostPos = distribution.argMax()
            ghostPositions.append(mostLikelyGhostPos)

        # Step 2: Find the position of the closest ghost
        ...
        # Step 3: Choose the action that brings Pacman closest to the nearest ghost's position
        bestAction = None
